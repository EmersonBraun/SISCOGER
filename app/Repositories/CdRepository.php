...
class CdRepository extends BaseRepository
{
    ...
    public function all()
	{
        ...
        {
            $registros = Cache::remember('todos_cd'.$unidade, self::$expiration, function() use ($unidade) {
    ...
    public function ano($ano)
	{
        ...
        {
            $registros = Cache::remember('todos_cd'.$ano.$unidade, self::$expiration, function() use ($unidade, $ano) {
    ...
    public function andamento()
	{
        ...
        {
            $registros = Cache::remember('andamento_cd'.$unidade, self::$expiration, function() use ($unidade) {
    ...
    public function andamentoAno($ano)
	{
        ...
        if($verTodasUnidades)
        {
            $registros = Cache::remember('andamento_cd'.$ano, self::$expiration, function() use ($ano){
        ...
        {
            $registros = Cache::remember('andamento_cd'.$ano.$unidade, self::$expiration, function() use ($unidade, $ano) {
                return $this->model->where('sjd_ref_ano', '=' ,$ano)
    ...
    public function julgamento()
	{
        ...
        {
            $registros = Cache::remember('julgamento_cd'.$unidade, self::$expiration, function() use ($unidade) {
                return $this->model->where('cdopm','like',$unidade.'%')
                    ->leftJoin('envolvido', function ($join){
                        $join->on('envolvido.id_cd', '=', 'cd.id_cd')
                                ->where('envolvido.id_cd', '<>', 0);
    ...
    public function julgamentoAno($ano)
	{
        ...
        if($verTodasUnidades)
        {
            $registros = Cache::remember('julgamento_cd'.$ano, self::$expiration, function() use ($ano){
                return $this->model->where('sjd_ref_ano', '=' ,$ano)
                    ->leftJoin('envolvido', function ($join) {
                        $join->on('envolvido.id_cd', '=', 'cd.id_cd')
                                ->where('envolvido.id_cd', '<>', 0);
        ...
        {
            $registros = Cache::remember('julgamento_cd'.$ano.$unidade, self::$expiration, function() use ($unidade,$ano) {
                return $this->model->where('sjd_ref_ano', '=' ,$ano)
                    ->where('cdopm','like',$unidade.'%')
                    ->leftJoin('envolvido', function ($join){
                        $join->on('envolvido.id_cd', '=', 'cd.id_cd')
    ...
    public function prazos()
    {
        ...
        if($verTodasUnidades)
        {
            ...
            $registros = Cache::remember('prazo_cd', self::$expiration, function() {
                return $this->model
                    ->selectRaw('cd.*, 
        ...
        {
            $registros = Cache::remember('prazo_cd'.$unidade, self::$expiration, function() use ($unidade){
                return $this->model
                    ->selectRaw('cd.*, 
                    (SELECT  motivo FROM sobrestamento WHERE sobrestamento.id_cd=cd.id_cd ORDER BY sobrestamento.id_sobrestamento DESC LIMIT 1) AS motivo,  
                    (SELECT  motivo_outros FROM sobrestamento WHERE   sobrestamento.id_cd=cd.id_cd ORDER BY sobrestamento.id_sobrestamento DESC LIMIT 1) AS motivo_outros, 
    ...
    public function prazosAno($ano)
    {
        ...
        if($verTodasUnidades)
        {
            ...
            $registros = Cache::remember('prazo_cd'.$ano, self::$expiration, function() use ($ano) {
                return $this->model
                    ->selectRaw('cd.*, 
                    (SELECT  motivo FROM sobrestamento WHERE sobrestamento.id_cd=cd.id_cd ORDER BY sobrestamento.id_sobrestamento DESC LIMIT 1) AS motivo,  
                    (SELECT  motivo_outros FROM sobrestamento WHERE   sobrestamento.id_cd=cd.id_cd ORDER BY sobrestamento.id_sobrestamento DESC LIMIT 1) AS motivo_outros, 
        ...
        {
            $registros = Cache::remember('prazo_cd'.$ano.$unidade, self::$expiration, function() use ($unidade, $ano){
                return $this->model
                    ->selectRaw('cd.*, 
                    (SELECT  motivo FROM sobrestamento WHERE sobrestamento.id_cd=cd.id_cd ORDER BY sobrestamento.id_sobrestamento DESC LIMIT 1) AS motivo,  
                    (SELECT  motivo_outros FROM sobrestamento WHERE   sobrestamento.id_cd=cd.id_cd ORDER BY sobrestamento.id_sobrestamento DESC LIMIT 1) AS motivo_outros, 
