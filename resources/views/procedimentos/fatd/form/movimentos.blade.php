...
<section class="content-header">   
  <h1>{{ strtoupper('fatd') }} - Movimentos</h1>
  <ol class="breadcrumb">
  <li><a href="{{route('home')}}"><i class="fa fa-dashboard"></i> Home</a></li>
  <li><a href="{{route('fatd.lista',['ano' => date('Y')])}}">{{ sistema('procedimentosTipos',strtoupper('fatd')) }} - Lista</a></li>
  <li><a href="{{route('fatd.edit',['ref' => $proc['sjd_ref'],'ano' => $proc['sjd_ref_ano']])}}">{{ sistema('procedimentosTipos',strtoupper('fatd')) }} - Editar</a></li>
  <li class="active">{{ sistema('procedimentosTipos',strtoupper('fatd')) }} - Movimentos</li>
  </ol>
  <br>
  <div class='form-group col-md-12 col-xs-12' style='padding-left: 0px'>
    <div class='btn-group col-md-8 col-xs-12 ' style='padding-left: 0px'>
        <a class="btn btn-default col-md-4 col-xs-4 "  href="{{route('fatd.edit',['ref' => $proc['sjd_ref'],'ano' => $proc['sjd_ref_ano']])}}">Editar</a>
        <a class="btn btn-success col-md-4 col-xs-4 "  href="{{route('fatd.movimentos',['ref' => $proc['sjd_ref'],'ano' => $proc['sjd_ref_ano']])}}">Movimentos</a>
        <a class="btn btn-default col-md-4 col-xs-4 "  href="{{route('fatd.sobrestamentos',['ref' => $proc['sjd_ref'],'ano' => $proc['sjd_ref_ano']])}}">Sobrestamentos</a>   
    </div>
...
<section class="">
        <div class="col-md-8">
        <h3>{{ nome_ext('fatd') }} - Nº {{ $proc['sjd_ref'] }} / {{ $proc['sjd_ref_ano'] }}</h3>
        </div>
    ...
    @component('components.movimento',['proc' => $proc, 'name' => 'fatd'])
    @endcomponent
