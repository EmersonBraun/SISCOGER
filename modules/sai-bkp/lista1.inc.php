...
a.andamento, ag.andamentocoger, ve.placa, ve.prefixo  
FROM sai
LEFT JOIN municipio AS m ON m.id_municipio=sai.id_municipio
LEFT JOIN andamento AS a ON a.id_andamento=sai.id_andamento
LEFT JOIN andamentocoger AS ag ON ag.id_andamentocoger=sai.id_andamentocoger 
LEFT JOIN viaturaenvolvida AS ve ON ve.id_sai=sai.id_sai
LEFT JOIN RHPARANA.opmPMPR AS rhopm ON rhopm.CODIGOBASE = sai.cdopm";
...
while ($row=mysql_fetch_array($res)) {
	...
	foreach ($mostrar as $campo) {
		...
		if     ($campo=="NRSJD") echo "<td><a href='?module=sai&sai[id]=$row[id_sai]'>$row[id_sai]</a></td>";
