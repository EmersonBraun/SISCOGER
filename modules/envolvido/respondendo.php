	...
	while ($row=mysql_fetch_assoc($res)) {
		...
		openTR("bgcolor='$cor'");
                        ...
                        if (empty($abreviatura)) {
                            $sqlInativo = sprintf("SELECT * FROM INATIVOS WHERE CBR_NUM_RG = '%s' AND ID_TIPO_RH = '02'", $row['rg']);
                            $resInativo = @mssql_query($sqlInativo, $con[4]);
                            if ($resInativo !== false && $rowInativo = mssql_fetch_assoc($resInativo)) {
                                $abreviatura = 'RESERVA';
                            }
                        }

                        if (empty($abreviatura)) {
                            $sqlExcluido = sprintf("SELECT * FROM
                                    (SELECT rg, nome FROM sjd.envolvido
                                    where resultado = 'Excluido' AND exclusaoportaria_data <> '0000-00-00' and
                                    (rg = '%s')
                                    UNION
                                    SELECT rg,nome FROM exclusaojudicial
                                    WHERE rg = '%s') AS excluidos", $row['rg'], $row['rg']);
                            $resExcluido = @mysql_query($sqlExcluido, $con[8]);
                            if ($resExcluido !== false && $rowExcluido = mysql_fetch_assoc($resExcluido)) {
                                $abreviatura = 'EXCLUÍDO';
                            }
                        }
