...
$sql="SELECT rg, cargo, nome, envolvido.id_envolvido, exclusaoportaria_data, exclusaobg_data, exclusaobg_numero, exclusaobg_ano, exclusaoportaria_numero, exclusaoopm,
	...
	cd.sjd_ref_ano as cd_ano, adl.sjd_ref_ano as adl_ano, cj.sjd_ref_ano as cj_ano,
        ...
        cd.id_decorrenciaconselho as cd_decorr,
        adl.id_decorrenciaconselho as adl_decorr,
        cj.id_decorrenciaconselho as cj_decorr,

        cd.id_situacaoconselho as cd_sit,
        adl.id_situacaoconselho as adl_sit,
        cj.id_situacaoconselho as cj_sit,
	...
	if ($ano) $sqlWhere[]=" (cd.sjd_ref_ano='$ano' OR cj.sjd_ref_ano='$ano' OR adl.sjd_ref_ano='$ano') ";
        ...
        $situacaoconselho = (integer)$_POST['situacaoconselho'];
        if (!empty($_POST['situacaoconselho'])) {
            $sqlWhere[] = " (cd.id_situacaoconselho='$situacaoconselho' OR cj.id_situacaoconselho='$situacaoconselho' OR adl.id_situacaoconselho='$situacaoconselho') ";
        }
...
form::openTR("id='linhafiltro' style='display:none;'");
	...
	form::closeTD();
		...
		form::openLabel("Situa&ccedil;&atilde;o");
			echo "<select name='situacaoconselho'>";
			include ("includes/option_situacaoconselho.php");
			echo "</select>";
		form::closeLabel();
	form::closeTD();
	openTD();
...
openTable("class='bordasimples' width='100%'");
	...
	openTR("bgcolor='#E0E0E0'");
		lista::td("Nome","nome");
		echo "<td>Procedimento</td><td>Situa&ccedil;&atilde;o</td><td>Decorr&ecirc;ncia</td><td>Portaria de Exclusao</td><td>Boletim Geral</td>";
...
$total=mysql_num_rows($res);
	...
	while ($rowDC=mysql_fetch_assoc($resDC)){
        ...
        //MONTA A TABELA DAS SITUAÇÕES E GUARDA EM UM VETOR, PROVAVELMENTE DA PRA FAZER COM OS JOINS E UNIONS MAS TERIA Q REESTRUTURAR.
	$sqlSC="SELECT * FROM situacaoconselho ";
	$resSC=mysql_query($sqlSC);
	while ($rowSC=mysql_fetch_assoc($resSC)){
		$sitcon["$rowSC[id_situacaoconselho]"]=$rowSC["situacaoconselho"];
	}
	//pre($sitcon);
	...
	while ($row=mysql_fetch_assoc($res)) {
		...
		openTR("bgcolor='$cor'");
			echo "<td><a href='?module=tramitacao&opcao=punicao&policial[rg]=$row[rg]' target='_blank'>$row[cargo] $row[nome]</a></td>";
			...
			closeTD();
                        ...
                        echo "<td>".$sitcon[$row[$proc."_sit"]]. "</td>";
