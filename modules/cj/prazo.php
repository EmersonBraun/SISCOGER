...
//sql do modulo
$sql="SELECT cj.id_cj, andamento, andamentocoger, 
(
		SELECT  motivo
		FROM    sobrestamento
	...
	b.dusobrestado, 
	(DIASUTEIS(abertura_data,DATE(NOW()))-IFNULL(b.dusobrestado,0)) AS diasuteis FROM cj
	LEFT JOIN
	(SELECT id_cj, SUM(DIASUTEIS(inicio_data, termino_data)+1) AS dusobrestado fROM sobrestamento
	...
	LEFT JOIN envolvido ON
		envolvido.id_cj=cj.id_cj AND envolvido.situacao='Presidente' AND rg_substituto=''
	LEFT JOIN andamento ON
		andamento.id_andamento=cj.id_andamento
	LEFT JOIN andamentocoger ON
		andamentocoger.id_andamentocoger=cj.id_andamentocoger
	";

//Filtro somente procedimentos do ano
$cj->sjd_ref_ano_eq=$_SESSION['sjd_ano'];

$_REQUEST['orderby']="cj.id_cj DESC";
...
h1("PRAZO DOS CONSELHOS DE JUSTIFICA&Ccedil;&Atilde;O");
openTable("width='100%' class='bordasimples'");
	openLine(8);
		h2("Calculo do prazo dos CJ - contado em dias uteis, conta-se o primeiro dia.<br> Sao descontados os dias em que o procedimento ficou sobrestado.<br> Data de refer&ecirc;ncia: HOJE (".date("d/m/Y").")");
	closeLine();
