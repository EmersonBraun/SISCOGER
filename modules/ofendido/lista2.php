...
if($proc != 'ipm') {
    unset($_REQUEST['ofendido']['crime']);
    unset($_REQUEST['ofendido']['vitima_eq']);
    unset($_REQUEST['ofendido']['confronto_armado_bl_eq']);
}
...
openTable("id='tabelafiltro' class='bordasimples' style='border-top:0px;' width='100%'");
	openTR();
		...
		openTD();
			...
			$sql="SELECT * FROM procedimentos_tipos where sigla = '{$proc}'";
			$res=mysql_query($sql);
			if ($row=mysql_fetch_assoc($res)) {
                               echo "<input type='hidden' name='proc' value='".strtolower($row["sigla"])."' />";
		...
		openTD();
			form::openLabel("Resultado");
			       echo "<input type='hidden' name='ofendido[resultado_eq]' value='".$ofendido->resultado_eq."'>";
                               echo "<strong>";
                               echo $ofendido->resultado_eq ? $ofendido->resultado_eq : "N&atilde;o cadastrado";
                               echo "</strong>";
			form::closeLabel();
		closeTD();
	closeTR();
