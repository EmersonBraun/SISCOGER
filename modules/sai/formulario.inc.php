...
if (!$sai->abertura_data) $sai->abertura_data=date("d/m/Y");
...
<table class='bordasimples' width='100%'>
	...
	<tr>
		...
		| <a href="?module=arquivo&arquivo[id_sai]=<?=$sai->id_sai;?>">Arquivo</a>
		</td>
	</tr>
	<?}?>
</tr>
...
form::openTR();
	...
	closeTD();
	openTD("width='33%'");
		form::input("Local de arquivo","sai[arquivopasta]");	
	closeTD();
form::closeTR();
...
openTR();
	openTD("width='33%'");
		form::openLabel("1&ordm; BOU (Boletim de Ocorr&ecirc;ncia Unificado)");
			echo "Ano: ";			
			echo "<select name='sai[bou_ano1]'>\r\n";
				for ($i=2005;$i<=date("Y");$i++) {
					echo "<option>$i</option>";
				}
			echo "</select>  N&ordm;";
			echo "<input name='sai[bou_numero1]' type='text' size='7' maxlength='7' onkeypress='return dntr(this, event);'>";
		form::closeLabel();
	closeTD();
	openTD("width='33%'");
		form::openLabel("2&ordm; BOU (Boletim de Ocorr&ecirc;ncia Unificado)");
			echo "Ano: ";			
			echo "<select name='sai[bou_ano2]'>\r\n";
				for ($i=2005;$i<=date("Y");$i++) {
					echo "<option>$i</option>";
				}
			echo "</select>  N&ordm;";
			echo "<input name='sai[bou_numero2]' type='text' size='7' maxlength='7' onkeypress='return dntr(this, event);'>";
		form::closeLabel();
	closeTD();
	openTD("width='33%'");
		form::openLabel("3&ordm; BOU (Boletim de Ocorr&ecirc;ncia Unificado)");
			echo "Ano: ";			
			echo "<select name='sai[bou_ano3]'>\r\n";
				for ($i=2005;$i<=date("Y");$i++) {
					echo "<option>$i</option>";
				}
			echo "</select>  N&ordm;";
			echo "<input name='sai[bou_numero3]' type='text' size='7' maxlength='7' onkeypress='return dntr(this, event);'>";
		form::closeLabel();
	closeTD();
closeTR();
	form::openTD("width='33%'");
...
form::openTR();
	form::openTD("class='col-lg-12 w-100'");
