...
<form class="controlar-modificacao" id='adl' name="adl" action="index.php?module=adl" method="post" onSubmit="return validar(this);" enctype="multipart/form-data">
...
preenchimento</font></th></tr> 	<?}?>
	...
	<tr>
        ...
        <a href="?module=subscription&tipo_processo=adl&id_processo=<?=$adl->id_adl;?>">Acompanhamento</a> 
        <?if ($user['nivel']==4 || $user['nivel']==5){?>
        | <a href="?module=arquivo&arquivo[id_adl]=<?=$adl->id_adl;?>">Arquivo</a>
        <?}?>
		</td>
	</tr>
	<?}?>
...
if ($opcao=="atualizar")
{
	...
	$sql="SELECT * FROM sobrestamento WHERE id_adl='".$adl->id_adl."' AND termino_data='0000-00-00'";
	if ($_SESSION[debug]) echo $sql."<br>\r\n";
...
form::openTR();
	form::openTD("colspan='2'");
		form::openLabel("Nº do ADL e andamento");
		if ($adl->sjd_ref) {
		...
		formulario::option("adl","andamento","WHERE procedimento='adl'","",0,0);
		form::closeLabel();
	form::closeTD();
	openTD();
		form::openLabel("Andamento COGER");
			formulario::option("adl","andamentocoger","WHERE procedimento='adl'","",0,0);
		form::closeLabel();
	closeTD();
form::closeTR();
...
openTR();
	openTD("colspan='3'");
		form::openLabel("Situação");
			formulario::option("adl","situacaoconselho","","",0,0);
		form::closeLabel();
	closeTD();
closeTR();
...
openTR();
	openTD("colspan='2'");
		form::openLabel("Em decorr&ecirc;ncia de");
			formulario::option("adl","decorrenciaconselho", "", "", 0,0);
		form::closeLabel();
	closeTD();
	openTD();
