...
//sql do modulo
$sql="SELECT it.*, andamento, rhopm.ABREVIATURA, encarregado.nome, encarregado.cargo, andamentocoger.andamentocoger FROM it
	LEFT JOIN andamento ON
		it.id_andamento = andamento.id_andamento
	LEFT JOIN andamentocoger ON
		it.id_andamentocoger = andamentocoger.id_andamentocoger
	LEFT OUTER JOIN RHPARANA.opmPMPR AS rhopm ON
		rhopm.CODIGOBASE=it.cdopm
	INNER JOIN encarregado_it AS encarregado ON
		encarregado.id_it=it.id_it
	";
//pre($sql);
...
openTable("width='100%' class='bordasimples'");
	openLine(6);
		h1("ANDAMENTO DOS it (".$opm_login->abreviatura.")");
	closeLine();
