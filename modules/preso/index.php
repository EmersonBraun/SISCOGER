...
if ($_REQUEST['preso']['id']) $opcao="atualizar";
$preso->setValues($_REQUEST['preso']);
if ($_SESSION['debug']) { echo "<pre>";print_r($preso); echo "</pre>"; }	
...
if (strtolower($opcao)=="cadastrar")  {
	if (strtolower($acao)=="cadastrar") {
		...
		echo "<a id='foco' href='?module=preso&opcao=listar'>Clique aqui para voltar.</a>";
	}
	else {
...
elseif ($opcao=="apagar") {
	if (preso::apaga($preso)) {
		echo "<h2>Preso apagado com sucesso!</h2>";
		echo "<a id='foco' href='?module=preso&opcao=listar'>Clique aqui para voltar à lista</a>";
	}
	else {
		echo "Houve um problema durante a execução da solicitação!";
	}
