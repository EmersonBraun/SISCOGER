...
elseif ($rg) { 
	...
	if ($_SESSION['debug']) pre($policial);
	//tamnho do código base
	if ($opm_login->codigo == '0000000000') {
		$len = 1;
	} else {
		$len = strlen($opm_login->codigoBase);
	}
	//$len = strlen($policial->opm->codigoBase);
	if ($_SESSION['debug']) pre("len: ".$len);
	//compatibilidade por causa das diferentes buscas pela OPM
	$OPMFicha = (!is_null($policial->opm->codigoBase) || $policial->opm->codigoBase != '') ? $policial->opm->codigoBase : $policial->opm2->codigoBase;
	if ($_SESSION['debug']) pre("OPM ficha: ".$OPMFicha);
	//if ($_SESSION['debug']) pre("OPM: ".$policial->opm->codigoBase);
	//if ($_SESSION['debug']) pre("OPM2: ".$policial->opm2->codigoBase);	
	//pegar o começo do código base para ver se é da mesma OPM
	$opmPM=substr($OPMFicha,0,$len);

	/*Gambiarra feia, pq não tava aparecendo a unidade do cabeça*/
	if ($rg =='106749353') $opmPM = '614';
	//pre("opm ficha: ".$OPMFicha);
	...
	//pode ver a ficha de inativo mesmo sendo nível 2 -> CRPM e DP
	$pode_ver = (($crpm || $dp_inativos || $bpec) && $inativo) ? 1 : 0;
	if ($_SESSION['debug']) pre("Pode ver: ".$pode_ver);

	//$policialAbrevOpm = is_null($policial->opm->abreviatura) && $policial->funcao == 'INATIVO' ? 'RESERVA' :  $policial->opm->abreviatura;
//antigo
	$policialAbrevOpm = $inativo == 1 ? 'RESERVA' :  $policial->opm->abreviatura;
	if ($_SESSION['debug']) pre("policialAbrevOpm: ".$policialAbrevOpm);
	
	if (!$pode_ver && $dp_inativos == 0 && $opm_login->abreviatura != 'CG') {
		...
		if ($user['nivel']<=2) {
			...
			similar_text($opmPM, $opm_login->codigoBase, $similaridade_entre_opms);
			if ($_SESSION['debug']) pre("Proximidade: ".$similaridade_entre_opms."%");
			//if ($opmPM != $opm_login->codigoBase && !($usuarioComandoIntermediario === true && $policial->funcao == 'INATIVO'))
			if ($similaridade_entre_opms < 80 && !($usuarioComandoIntermediario === true && $policial->funcao == 'INATIVO')) 
			{

				echo "<h3>Acesso negado! Voce s&oacute; pode ver policiais da sua OPM (".$opm_login->abreviatura.")</h3>";
				echo "<br>O policial que voce tentou acessar 
				(<b>".$policial->cargo." ".$policial->nome."</b>) 
				est&aacute; classificado no(a) <b>".$policialAbrevOpm."</b><br><br>";
	...
	openTR();
		openTD();
			form::openLabel("Classificacao Meta4");
				if($policial->opm_desc){
					echo $policial->opm_desc;
				}else{
	...
	openTR("id='denuncia'");
		openTD("colspan='3'");
			$texto="Cadastrar den&uacute;ncia criminal ou condena&ccedil;&atilde;o em decorr&ecirc;ncia de IPM, APFD, 
			e deser&ccedil;&atilde;o, cadastrar o militar no campo &#34;R&eacute;us&#34;, dentro de cada planilha 
			(IPM, APFD e DESER&Ccedil;&Atilde;O) existente no procedimento correspondente, cadastrando as principais informa&ccedil;&otilde;es 
			dentro da planilha, al&eacute;m do tipo do crime, data da den&uacute;ncia, tipos penais condenat&oacute;rios e a data do tr&acirc;nsito 
			em julgado. Dever&aacute; ainda constar o processo crime e a Comarca para acompanhamento no site do TJPR.";
	...
	<tr style='padding:0px;'><td>
		...
		<?php
			if ($user['nivel']==4 || $user['nivel']==5){
				menuli("Protocolo","protocolo&policial[rg]=$rg");
			}
			if ($user['nivel']==5 || $user['nivel']==7)
				menuli("Dili&ecirc;gncias","sai&policial[rg]=$rg");
				menuli("SAI","envolvido&policial[rg]=$rg");
            ...
            if ($user['nivel']==2 || $user['nivel']==4 || $user['nivel']==5 || $user['nivel']==7)
				menuli("Outros","proc_outros&policial[rg]=$rg");
