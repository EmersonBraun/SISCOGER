...
elseif ($rg) {
	...
	if ($user['nivel']<=2) {
		...
		if ($opmPM != $opm_login->codigoBase) {
			echo "<h3>Acesso negado! Voce s&oacute; pode ver policiais da sua OPM (".$opm_login->abreviatura.")</h3>";
			echo "<br>O policial que voce tentou acessar (<b>".$policial->cargo." ".$policial->nome."</b>) est&aacute; classificado no(a) <b>".$policial->opm->abreviatura."</b><br><br>";
	...
	if ($user['nivel']<4) {
		if ($usuario->id_posto > 7 and $policial->id_posto <= 7) {
	...
	//QUERY DE SUB JUDICE
	$sqlSUB="SELECT * FROM envolvido WHERE rg='".$policial->rg."' AND ipm_processocrime='Denunciado'";
