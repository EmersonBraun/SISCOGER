...
<form class="controlar-modificacao" ID='sindicancia' name="sindicancia" action="index.php?module=sindicancia" method=post onSubmit="return validar(this);" enctype="multipart/form-data">
...
<table class='bordasimples' width='100%'>
	...
	<tr>
        ...
        <a href="?module=subscription&tipo_processo=sindicancia&id_processo=<?=$sindicancia->id_sindicancia;?>">Acompanhamento</a>
        <?if ($user['nivel']==4 || $user['nivel']==5){?>
        | <a href="?module=arquivo&arquivo[id_sindicancia]=<?=$sindicancia->id_sindicancia;?>">Arquivo</a>
        <?}?>
		</td>
	</tr>
	<?}?>
...
if ($opcao=="atualizar")
{
	...
	$sql="SELECT * FROM sobrestamento WHERE id_sindicancia='".$sindicancia->id_sindicancia."' AND termino_data='0000-00-00'";
...
if ($user['nivel']==4 || $user['nivel']==5)
{
	form::openTD("colspan='5'");
		...
		<select name='sindicancia[prioridade]'>
...
form::openTR();
	form::openTD("colspan='2' width='50%'");
		form::input("Relat&oacute;rio do Encarregado","sindicancia[relatorio_file]");
	form::closeTD();
	form::openTD("colspan='1' width='50%'");
		form::input("Data do Relat&oacute;rio","sindicancia[relatorio_data]");
	form::closeTD();
form::closeTR();
